...
function timeCount() {
    const date = _dateFns.add(new Date(input.value), {
        hours: -7
    });
    let hour = _dateFns.formatDistanceToNowStrict(date, {
        unit: "hour"
    }).slice(0, -5);
    let year = _dateFns.formatDistanceToNowStrict(date, {
        unit: "year"
    }).slice(0, -5);
    let day = _dateFns.formatDistanceToNowStrict(date, {
        unit: "day"
    }).slice(0, -4);
    let obj = {
        years: +year,
    ...
    let untilDate = _dateFns.formatDuration(obj);
    output.value = `${untilDate}` + " until " + format(date, "dd/MM/yyyy");
}
...
},{"../_lib/requiredArgs/index.js":"9wUgQ","../constants/index.js":"iOhcx","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["huMbS","bDbGG"], "bDbGG", "parcelRequire5aa1")
